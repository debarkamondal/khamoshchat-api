...
pub fn decode_b64_key(base64_str: &str, expected_len: usize, name: &str) -> Result<Vec<u8>, AppError> {
...
pub fn verify_signed_prekey(
    ...
    let pre_key: [u8; 33] = signed_prekey_bytes.try_into().unwrap();
    let sig: [u8; 96] = signature_bytes.try_into().unwrap();

    match vxeddsa_verify(&i_key, &pre_key, &sig) {
        Some(_) => Ok(()),
